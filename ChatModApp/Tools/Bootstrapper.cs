...
public static class Bootstrapper
{
    public static void Init()
    {
        ...
        }).UseEnvironment(Environments.Development).Build();
    ...
    private static void ConfigureServices(IServiceProvider services)
    {
        ...
        var loggerFactoryMethod = typeof(LoggerFactoryExtensions).GetMethods(BindingFlags.Static | BindingFlags.Public)
                                                                 .First(info => info.Name ==
                                                                                nameof(LoggerFactoryExtensions
                                                                                           .CreateLogger) &&
                                                                                info.IsGenericMethod);

        container.Register(typeof(ILogger<>),
                           made: Made.Of(
                               req => loggerFactoryMethod.MakeGenericMethod(req.ServiceType.GenericTypeArguments.First())));
