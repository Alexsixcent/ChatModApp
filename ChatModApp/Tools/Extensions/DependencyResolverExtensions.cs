...
public static class DependencyResolverExtensions
{
    ...
    public static void RegisterViewsForViewModels(this IContainer container, Assembly assembly, string parentNamespace = "")
    {
        ...
        var typeInfos = assembly.DefinedTypes.Where(ti => ti.Namespace is not null &&
                                                          ti.Namespace.StartsWith(parentNamespace) &&
                                                          !ti.Name.EndsWith("Base") &&
                                                          ti.GetTypeInfo().ImplementedInterfaces
                                                            .Contains(typeof(IViewFor)) && !ti.IsAbstract);
        ...
        foreach (var ti in typeInfos)
        {
            ...
            var ivf = ti.ImplementedInterfaces.FirstOrDefault(
                t => t.GetTypeInfo().ImplementedInterfaces.Contains(typeof(IViewFor)));

            // need to check for null because some classes may implement IViewFor but not IViewFor<T> - we don't care about those
            if (ivf is null) continue;
    ...
    private static void RegisterType(IRegistrator container, TypeInfo ti, Type serviceType, string contract)
    {
        var constructor = GetConstructor(ti);

        container.Register(serviceType,
                           ti.UnderlyingSystemType,
    ...
    private static ConstructorInfo GetConstructor(TypeInfo typeInfo)
    {
        ...
        if (parameterlessConstructor is null)
            throw new($"Failed to register type {typeInfo.FullName} because it's missing a parameter-less constructor.");
