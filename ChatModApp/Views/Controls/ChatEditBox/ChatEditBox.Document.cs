...
/// <summary>
///     The RichSuggestBox control extends <see cref="ChatEditBox" /> control that suggests and embeds custom data in a
///     rich document.
/// </summary>
public partial class ChatEditBox
{
    ...
    private void ExpandSelectionOnPartialTokenSelect(ITextSelection selection, ITextRange tokenRange)
    {
        switch (selection.Type)
        {
            case SelectionType.InsertionPoint:
                ...
                if (tokenRange.StartPosition < selection.StartPosition &&
                    selection.EndPosition < tokenRange.EndPosition)
                {
            ...
            case SelectionType.Normal:
                ...
                if (tokenRange.StartPosition <= selection.StartPosition &&
                    selection.EndPosition < tokenRange.EndPosition ||
                    tokenRange.StartPosition < selection.StartPosition &&
                    selection.EndPosition <= tokenRange.EndPosition)
                {
    ...
    {
        if (TokenSelected == null || !TryGetTokenFromRange(selection, out var token) ||
            token.RangeEnd != selection.EndPosition)
            return;

        TokenSelected.Invoke(this,
                             new()
                             {
                                 Token = token,
    ...
    {
        foreach (var (_, token) in _tokens) token.Active = false;
    ...
    {
        if (range.Length == 0 || !TryGetTokenFromRange(range, out var token)) return;
        ...
        if (token.Active && token.RangeStart != range.StartPosition && token.RangeEnd != range.EndPosition)
        {
            ...
            if (TryCommitSuggestionIntoDocument(range, token.DisplayText,null, null, guid, CreateTokenFormat(range),
                                                      false))
            {
                token = new(guid, token.DisplayText) { Active = true, Item = token.Item };
    ...
    private bool TryCommitSuggestionIntoDocument(ITextRange range,
                                                             string displayText,
    ...
    {
        if (imageStream is not null && imageDecoder is not null)
        {
            range.SetText(TextSetOptions.None, displayText);
            range.InsertImage((int)imageDecoder.PixelWidth, (int)imageDecoder.PixelHeight, 0,
                              VerticalCharacterAlignment.Baseline, displayText, imageStream);
        }
        else
        {
            ...
            if (existingText != displayText) range.SetText(TextSetOptions.Unhide, displayText);
        }
    ...
    private bool TryExtractQueryFromSelection(out string prefix, out string query, out ITextRange range)
    {
        ...
        query = string.Empty;
        range = null;
        if (TextDocument.Selection.Type != SelectionType.InsertionPoint) return false;
        ...
        range = selection;
        if (TryExtractQueryFromRange(selection, out prefix, out query)) return true;
    ...
    private bool TryExtractQueryFromRange(ITextRange range, out string prefix, out string query)
    {
        ...
        if (possibleQuery.Length > 0 && Prefixes.Contains(possibleQuery[0]) && !possibleQuery.Any(char.IsWhiteSpace) &&
            string.IsNullOrEmpty(range.Link))
        {
    ...
    private ITextCharacterFormat CreateTokenFormat(ITextRange range)
    {
        ...
        if (TokenBackground != null) format.BackgroundColor = TokenBackground.Color;

        if (TokenForeground != null) format.ForegroundColor = TokenForeground.Color;
