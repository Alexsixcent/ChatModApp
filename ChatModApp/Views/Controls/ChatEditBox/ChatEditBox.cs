...
public partial class ChatEditBox : ItemsControl
{
    ...
    private ChatEditQuery _currentQuery;
    private readonly Dictionary<string, ChatSuggestToken> _tokens;

    private int _suggestionChoice;
    private ListViewBase? _suggestionsList;

    private readonly object _tokensLock;
    private readonly ObservableCollection<ChatSuggestToken> _visibleTokens;
    ...
    public ChatEditBox()
    {
        ...
        _tokens = new();
        _visibleTokens = new();
        Tokens = new(_visibleTokens);

        DefaultStyleKey = typeof(ChatEditBox);
    ...
    public void ClearUndoRedoSuggestionHistory()
    {
        ...
        lock (_tokensLock)
        {
            ...
            foreach (var key in keysToDelete) _tokens.Remove(key);
        }
    }
    ...
    public void AddTokens(IEnumerable<ChatSuggestToken> tokens)
    {
        lock (_tokensLock)
        {
            foreach (var token in tokens) _tokens.TryAdd($"\"{token.Id}\"", token);
        }
    }

    /// <summary>
    ///     Populate the <see cref="RichSuggestBox" /> with an existing Rich Text
    ///     Format (RTF) document and a collection of tokens.
    /// </summary>
    ...
    public void Load(string rtf, IEnumerable<ChatSuggestToken> tokens)
    {
        Clear();
        AddTokens(tokens);
    ...
    /// <param name="token">
    ///     When this method returns, contains the token associated with the specified range; otherwise, it is
    ///     null.
    /// </param>
    /// <returns>true if there is a token associated with the text range; otherwise false.</returns>
    public bool TryGetTokenFromRange(ITextRange range, out ChatSuggestToken token)
    {
        token = null;
    ...
    ///     with position measured from the top left of the
    ///     <see cref="RichSuggestBox" /> control.
    /// </summary>
    ...
    private static void OnPrefixesChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (newValue != prefixes) view.SetValue(PrefixesProperty, prefixes);
    }
    ...
    {
        if (_suggestionPopup.IsOpen) UpdatePopupOffset();
    }
    ...
    private void RichEditBox_OnPointerMoved(object sender, PointerRoutedEventArgs e)
    {
        ...
        if (TokenPointerOver != null) InvokeTokenPointerOver(pointer);
    }
    ...
    private void RichEditBox_SelectionChanging(RichEditBox sender, RichEditBoxSelectionChangingEventArgs args)
    {
        ...
        if (selection.Type != SelectionType.InsertionPoint && selection.Type != SelectionType.Normal) return;

        var range = selection.GetClone();
        range.Expand(TextRangeUnit.Link);
        lock (_tokensLock)
        {
            if (!_tokens.ContainsKey(range.Link)) return;
        }
    ...
    private async void RichEditBox_SelectionChanged(object sender, RoutedEventArgs e)
    {
        ...
        if (_textCompositionActive) return;
    ...
    private async void RichEditBox_ProcessKeyboardAccelerators(UIElement sender, ProcessKeyboardAcceleratorEventArgs args)
    {
        ...
        if (!_suggestionPopup.IsOpen || itemsList == null || itemsList.Count == 0) return;
    ...
    {
        if (_ignoreChange || !args.IsContentChanging) return;

        lock (_tokensLock)
        {
            CreateSingleEdit(ValidateTokensInDocument);
        }
    }
    ...
    private async void RichEditBox_Paste(object sender, TextControlPasteEventArgs e)
    {
        ...
        if (e.Handled || TextDocument == null || ClipboardPasteFormat != RichEditClipboardFormat.PlainText) return;
    ...
    private void UpdateVisibleTokenList()
    {
        lock (_tokensLock)
        {
            ...
            foreach (var elem in toBeRemoved) _visibleTokens.Remove(elem);

            var toBeAdded = _tokens.Where(pair => pair.Value.Active && !_visibleTokens.Contains(pair.Value))
                                   .Select(pair => pair.Value).ToArray();

            foreach (var elem in toBeAdded) _visibleTokens.Add(elem);
        }
    }
}