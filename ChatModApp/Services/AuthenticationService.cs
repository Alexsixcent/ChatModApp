...
public class AuthenticationService
{
    ...
    public AuthenticationService()
    {
        ...
        var connectable = Observable
                          .FromEventPattern<string>(h => AccessTokenChangedEvent += h, h => AccessTokenChangedEvent -= h)
                          .Select(pattern => pattern.EventArgs).Replay(1);
    ...
    public (Uri Uri, TwitchAuthQueryParams QueryParams) GenerateAuthUri()
    {
        ...
        return (new Uri("https://id.twitch.tv/oauth2/authorize").AddQueries(queryParams), queryParams);
    }

    public bool AuthFromCallbackUri(Uri callbackUri)
    {
        if (!callbackUri.AbsoluteUri.Contains(RedirectUri)) return false;
