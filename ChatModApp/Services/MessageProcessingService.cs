...
public class MessageProcessingService
{
    ...
    private IEnumerable<IChatBadge> GetMessageBadges(string channel, IEnumerable<KeyValuePair<string, string>> badgePairs)
    {
        ...
        foreach (var (setId, id) in badgePairs)
            badges.AddRange(_chatService.ChatBadges.Items.Where(chatBadge => chatBadge.SetId == setId && chatBadge.Id == id)
                                        .Where(badge => badge.Channel is null || badge.Channel.Login == channel));
    ...
    private IEnumerable<IMessageFragment> GetMessageFragments(ChatMessage chatMessage)
    {
        ...
        else
        {
            ...
            if (lastEndIndex < msg.Length - 1)
                fragments.AddRange(ParseTextFragment(msg.Substring(lastEndIndex), chatMessage.Channel, endSpace: false));
        }

        return fragments;
    }

    private IEnumerable<IMessageFragment> ParseTextFragment(string msg,
                                                            string channel,
                                                            ...
                                                            bool endSpace = true)
    {
        ...
        foreach (var frag in msg.Split(' ', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (HasValidHost(frag) && Uri.TryCreate(frag, UriKind.RelativeOrAbsolute, out var uriRes))
            {
        ...
        {
            if (!startSpace && fragments.First() is TextFragment firstFrag) firstFrag.Text = firstFrag.Text.TrimStart(' ');

            if (!endSpace && fragments.Last() is TextFragment lastFrag) lastFrag.Text = lastFrag.Text.TrimEnd(' ');
        }
    ...
    {
        var host = uri.TrimStart("http://").TrimStart("https://").Split('/', StringSplitOptions.RemoveEmptyEntries)
                      .FirstOrDefault();

        if (string.IsNullOrEmpty(host)) return false;

        if (uri.HasConsecutiveChar(".")) return false;

        var i = host.LastIndexOf('.');

        return i > 0 && !uri.StartsWith('.') && !uri.EndsWith('.') &&
    ...
    private static Color GetColorFromTwitchHex(string hex) =>
        string.IsNullOrWhiteSpace(hex) ? Color.Gray : Color.FromArgb(int.Parse(hex.TrimStart(1), NumberStyles.HexNumber));
}