...
using EmoteKeyValue
    = System.Collections.Generic.KeyValuePair<ChatModApp.Models.Chat.Emotes.EmoteKey, ChatModApp.Models.Chat.Emotes.IEmote>;
...
public class EmotesService : IDisposable
{
    ...
    public IObservableCache<Grouping<Grouping<IEmote, string, EmoteKey.EmoteType>, EmoteKey.EmoteType, string>, string>
        UserEmoteGroups { get; }
    ...
    public EmotesService(TwitchApiService apiService,
                         ...
                         IFfzApi ffzApi)
    {
        ...
        _apiService.UserConnected.SelectMany(LoadGlobalEmotes).Subscribe().DisposeWith(_disposable);

        _chatService.ChannelsJoined.OnItemAdded(async s => await LoadChannelEmotes(s)).OnItemRemoved(UnloadChannelEmotes)
                    .Subscribe().DisposeWith(_disposable);

        GlobalEmotes = _emotes.Connect().Filter(kv => kv.Key.Type.HasFlag(EmoteKey.EmoteType.Global))
                              .ChangeKey(kv => kv.Key.Code).Transform(kv => kv.Value).AsObservableCache();

        UserEmotes = _emotes.Connect().Filter(kv => kv.Key.Type.HasFlag(EmoteKey.EmoteType.Member))
                            .Group(kv => kv.Key.Channel)
                            .Transform(group => new Grouping<IEmote, string, string>(
                                           group.Key,
                                           group.Cache.Connect().ChangeKey(kv => kv.Value.Code).Transform(pair => pair.Value)
                                                .AsObservableCache())).DisposeMany().AsObservableCache();


        GlobalEmoteGroups = _emotes.Connect().Filter(kv => kv.Key.Type.HasFlag(EmoteKey.EmoteType.Global))
                                   .Group(kv => kv.Key.Type).Transform(
                                       group =>
                                           (IGroup<IEmote, string, EmoteKey.EmoteType>)new
                                               Grouping<IEmote, string, EmoteKey.EmoteType>(
                                                   ...
                                                   group.Cache.Connect().ChangeKey(kv => kv.Value.Code)
                                                        .Transform(pair => pair.Value).AsObservableCache())).DisposeMany()
                                   .AsObservableCache();

        UserEmoteGroups = _emotes.Connect().Filter(kv => kv.Key.Type.HasFlag(EmoteKey.EmoteType.Member))
                                 .Group(kv => kv.Key.Channel).Transform(
                                     group =>
                                         new Grouping<Grouping<IEmote, string, EmoteKey.EmoteType>, EmoteKey.EmoteType,
                                             string>(group.Key,
                                                     group.Cache.Connect().Group(kv => kv.Key.Type).Transform(
                                                              innerGroup =>
                                                                  new Grouping<IEmote, string, EmoteKey.EmoteType>(
                                                                      innerGroup.Key,
                                                                      innerGroup.Cache.Connect()
                                                                                .ChangeKey(kv => kv.Value.Code)
                                                                                .Transform(kv => kv.Value)
                                                                                .AsObservableCache()))
                                                          .DisposeMany().AsObservableCache())).DisposeMany()
                                 .AsObservableCache();
        ...
        _emotes.Edit(updater =>
        {
            if (res2.IsSuccessStatusCode)
            {
                updater.AddOrUpdate(res2.Content.ChannelEmotes.Select(
                                        emote => new EmoteKeyValue(new(EmoteKey.EmoteType.Bttv | EmoteKey.EmoteType.Member,
                                                                       emote,
                                                                       channel.Login),
                                                                   emote)));
            ...
            if (res3.IsSuccessStatusCode)
                updater.AddOrUpdate(res3.Content.Sets.SelectMany(pair => pair.Value.Emoticons).Select(
                                        emote => new EmoteKeyValue(
                                            new(EmoteKey.EmoteType.FrankerZ | EmoteKey.EmoteType.Member,
                                                emote,
                                                channel.Login),
                                            emote)));
        });
    ...
    private void UnloadChannelEmotes(ITwitchChannel channel)
    {
        ...
        var emotes = group.Value.Cache.Items.SelectMany(
    ...
    private async Task<Unit> LoadGlobalEmotes(User user, CancellationToken cancel)
    {
        ...
        var globalTwitchEmotes = from emoteSet in globalEmotes.EmoteSets
        ...
        _emotes.Edit(updater =>
        {
            ...
            updater.AddOrUpdate(globalTwitchEmotes.Select(
                                    emote => new EmoteKeyValue(
                                        new(EmoteKey.EmoteType.Twitch | EmoteKey.EmoteType.Global, emote),
                                        emote)));
            updater.AddOrUpdate(globalBttvEmotes.Select(
                                    emote => new EmoteKeyValue(
                                        new(EmoteKey.EmoteType.Bttv | EmoteKey.EmoteType.Global, emote),
                                        emote)));
            updater.AddOrUpdate(globalFfzEmotes.Select(
                                    emote => new EmoteKeyValue(
                                        new(EmoteKey.EmoteType.FrankerZ | EmoteKey.EmoteType.Global, emote),
                                        emote)));
        });
